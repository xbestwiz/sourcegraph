...
var isValidMatcher = lazyregexp.New(`\.(s|sh|c|cs|css|dart|clj|elm|erl|ex|f|fsx|go|html|hs|java|js|json|jl|kt|tex|lisp|nim|ml|pas|php|py|re|rb|rs|scala|sql|swift|txt|ts)$`)
...
func structuralSearchZip(ctx context.Context, zipPath, pattern, rule, extension string, languages, includePatterns []string, excludePattern string, repo api.RepoName) (matches []protocol.FileMatch, limitHit bool, err error) {
  ...
  defer os.RemoveAll(dirPath)
	...
	return structuralSearchDir(ctx, dirPath, pattern, rule, extension, languages, includePatterns, repo, true)
}

func structuralSearchDir(ctx context.Context, dirPath, pattern, rule, extension string, languages, includePatterns []string, repo api.RepoName, filterWithRipgrep bool) (matches []protocol.FileMatch, limitHit bool, err error) {
	log15.Info("structural search", "repo", string(repo))
...
func structuralSearchWithZoekt(ctx context.Context, p *protocol.Request) (matches []protocol.FileMatch, limitHit, deadlineHit bool, err error) {
	...
	matches, limitHit, err = structuralSearchDir(ctx, contentDir, p.Pattern, p.CombyRule, extension, p.Languages, []string{}, p.Repo, false)
